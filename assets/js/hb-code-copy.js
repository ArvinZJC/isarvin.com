...
async function copyCodeToClipboard(button, codeWrapper) {
  // A temp solution to remove unexpected button text from the end of copied code.
  const codeToCopy = codeWrapper.textContent.endsWith(i18n["copy"])
    ? codeWrapper.textContent.slice(0, -i18n["copy"].length)
